...
public class HashBrowns {
    ...
    public ArrayList<TreeNodeBase> fullHashSearch(String input, GeneralTree GenTree){
        ...
        for(int i = 0; i<allAddresses.size(); i++){
            ...
            TreeNode tmp = GenTree.get(address);
            hits.add(tmp);
        }
        ...
        //---------------------------OPTIMIZATION---------------------------------//
        //The nodes we're personally interested in for GeneralTree-Noun are the ones that 'exist'.
        //In GeneralTree's architecture they're the ones at the second dimension level. GeneralTree/Noun.txt/TERM
        ...
        for(int i =0; i<basePrep.size(); i++){
            ...
            for(int j=0; j<terms.length; j++){
                if(tmp.containsAll(terms[j]) != null){
                    matchedTerms++;
                }
            }
